            ...
            {tab==="macros"&&(
              <div className="page fade-in">
                ...
                <div className="card" style={{marginBottom:12}}>
                  ...
                  {foodSearchResults.length>0&&<div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:8}}>{foodSearchResults.map((f,i)=><button key={i} className="food-pick-btn" style={{width:"100%",textAlign:"left"}} onClick={()=>{addQuickFood(f);setFoodSearch("");setFoodSearchResults([]);setAddingFood(false);}}><div className="fpb-name">{f.name}</div><div className="fpb-macros"><span style={{color:"#FF5733"}}>{f.cal}kcal</span> <span style={{color:"#3B82F6"}}>{f.protein}p</span> <span style={{color:"#F59E0B"}}>{f.carbs}c</span></div></button>)}</div>}
